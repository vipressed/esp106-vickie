...
```{r question 1}
tide_gauges <- data.frame(city=c("Boston", "New York", "Baltimore", "Charleston", "Miami", "Corpus Christi"), gauge_id = c(8443970, 8518750, 8574680, 8665530, 8723214, 8775296))
```
...
ggplot(tidedata, aes(x = Date, y = MHHW, color = city)) + geom_line() + geom_smooth(aes(x = Date, y = MHHW), method = "lm", inherit.aes = FALSE, color = "black") + labs(x = "Date", y = "Mean Higher High Water (m)", color = "City") + theme_minimal()
...
ggplot(tidedata, aes(x = Date, y = MHHW)) + geom_line(color = "steelblue") + geom_smooth(method = "lm", color = "black") + facet_wrap(~ city, scales = "free_y") + labs(x = "Date", y = "Mean Higher High Water (m)") + theme_minimal()
...
```{r question6}
ggplot(tidedata, aes(x = city, y = Highest)) + geom_boxplot() + labs(x = "City",y = "Highest Monthly Tide (m)") + theme_minimal()
```
...
outlier_row <- tidedata[which.max(tidedata$Highest),] outlier_row
```
...
# The estimated coefficient of the date column is 2.732e^-5 m/day and it is statistically significant.

# The mean increase in sea-level over the next 10 years (2011-2020) is approximately 0.0997 meters across the six cities.
